...
rounded-2xl shadow-2xl overflow-hidden border border-gray-200 flex flex-col max-h-[y0vh]">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-sm">✌️</div>
            <h2 className="text-base sm:text-lg font-bold text-gray-900">Add Signature</h2>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-1.5 rounded-lg hover:bg-gray-100 transition cursor-pointer" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex border-b border-gray-200 bg-gray-50 px-3 sm:px-6 pt-2 overflow-x-auto no-scrollbar shrink-0">
          <button onClick={() => setActiveTab('draw')} className={'flex items-center space-x-1.5 sm:space-x-2 px-3 sm:px-4 py-2 sm:py-2.5 font-medium text-xs sm:text-sm border-b-2 transition cursor-pointer shrink-0 ' + (activeTab === 'draw' ? 'border-emerald-500 text-emerald-600 bg-white rounded-t-lg' : 'border-transparent text-gray-500 hover:text-gray-700')}>
          ...
          {activeTab === 'type' && (
            <div className="space-y-3 sm:space-y-4">
              <div>
                ...
                <input type="text" value={typedName} onChange={(e) => setTypedName(e.target.value)} placeholder="Type name here..." className="w-full px-3 sm:px-3.5 py-2 sm:py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5 pt-1 sm:pt-2">
                {[
                  { id: 'font-cursive-1', fontFace: 'Dancing Script, cursive' },
                  { id: 'font-cursive-2', fontFace: 'Great Vibes, cursive' },
                  { id: 'font-cursive-3', fontFace: 'Caveat, cursive' },
                  { id: 'font-cursive-4', fontFace: 'Sacramento, cursive' },
                ].map((fontOption) => (
                  <button key={fontOption.id} onClick={() => selectedFont !== fontOption.id && setSelectedFont(fontOption.id)} style={{ color: selectedColor }} className={'p-2.5 sm:p-3 rounded-xl border text-center transition cursor-pointer ' + (selectedFont === fontOption.id ? 'border-emerald-500 bg-emerald-50/40 ring-1 ring-emerald-500' : 'border-gray-200 hover:border-gray-300 bg-white')}>
                    <div className="text-lg sm:text-xl truncate tracking-wide" style={{ fontFamily: fontOption.fontFace, fontStyle: 'italic' }}>
                      {typedName || 'Signature'}
                    </div>
                  </button>
                ))}
          ...
          {activeTab === 'upload' && (
            <div className="space-y-3">
              <label className="border-2 border-dashed border-gray-300 rounded-xl p-5 sm:p-6 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-400 bg-gray-50/50 transition text-center">
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-700">{uploadedImage ? 'Change signature image' : 'Upload PNG or JPG signature'}</span>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end space-x-2 sm:space-x-3 shrink-0">
          <button onClick={onClose} className="px-3.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-200/60 rounded-lg transition cursor-pointer">Cancel</button>
          <button onClick={handleSave} className="px-4 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg shadow-sm transition flex items-center space-x-1.5 cursor-pointer active:bg-emerald-700">
